...
// .wrangler/tmp/bundle-E6buj7/checked-fetch.js
...
- Always stay on topic with the article`;
      ...
      const { data: post2, error: fetchError } = await supabase.from("nucleus_posts").select("*").eq("id", postId).single();
      if (fetchError || !post2) {
      ...
      if (type === "summary") {
        if (post2.ai_summary) {
          return new Response(JSON.stringify({ result: post2.ai_summary }), {
            headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
          });
        }
...
${post2.content.replace(/\u003c[^\u003e]*\u003e?/gm, "")}`;
      ...
      const { error: updateError } = await supabase.from("nucleus_posts").update(updateData).eq("id", postId);
      if (updateError) {
...
// .wrangler/tmp/bundle-E6buj7/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-E6buj7/middleware-loader.entry.ts
